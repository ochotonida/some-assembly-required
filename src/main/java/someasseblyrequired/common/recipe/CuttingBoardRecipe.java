...
public class CuttingBoardRecipe implements IRecipe<RecipeWrapper> {
    ...
    public static class Serializer extends ForgeRegistryEntry<IRecipeSerializer<?>> implements IRecipeSerializer<CuttingBoardRecipe> {
        ...
        @Override
        public CuttingBoardRecipe read(ResourceLocation id, JsonObject json) {
            ...
            if (ingredients.size() != 1) {
                ...
                return new CuttingBoardRecipe(id, group, ingredients.get(0), result);
            }
        }

        @Nullable
        @Override
        public CuttingBoardRecipe read(ResourceLocation id, PacketBuffer buffer) {
            String group = buffer.readString(32767);
            Ingredient input = Ingredient.read(buffer);
            ItemStack result = buffer.readItemStack();

            return new CuttingBoardRecipe(id, group, input, result);
        }
